						...
						<?php foreach ($this->items as $item) : ?>
							...
							<tr class="row<?= $i++ % 2; ?>">
								...
								<td>
									...
									<?php
										$duration = null;
										if (($item->status === 'complete') && !empty($item->scanend) && ($item->scanend != Factory::getDbo()->getNullDate()) && $duration !== null) {
											try {
												$duration = (new Date($item->scanend))->diff(new Date($item->scanstart));
											} catch (Exception $e) {
												$duration = null;
											}
										}
