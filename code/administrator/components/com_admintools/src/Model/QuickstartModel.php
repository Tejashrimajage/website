...
class QuickstartModel extends BaseDatabaseModel
{
	...
	private function applyHtmaker()
	{
		...
		$isHSTS = $this->isLocalhost(true) ? false : Uri::getInstance()->getScheme() == 'https';
	...
	private function getSiteBaseDir(): string
	{
		$app       = Factory::getApplication();
		$live_site = trim($app->get('live_site') ?: '');
		$uri       = empty($live_site) ? Uri::getInstance() : Uri::getInstance($live_site);
		...
		// Remove the application-specific paths: installation, administrator, api (we should only need administrator)
		if (substr($sitePath, 0, 12) === 'installation')
		{
			$sitePath  = substr($sitePath, 12);

		}
		elseif (substr($sitePath, 0, 13) === 'administrator')
		{
			$sitePath = substr($sitePath, 13);
		}
		elseif (substr($sitePath, 0, 3) === 'api')
		{
			$sitePath = substr($sitePath, 3);
		}
		$sitePath  = '' . trim($sitePath, '/');
		$sitePath  = $sitePath ?: '/';
