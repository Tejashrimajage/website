...
class ScansModel extends ListModel
{
	...
	public function purgeFilesCache()
	{
		try
		{
			$this->getDbo()->truncateTable('#__admintools_filescache');
	...
	protected function getListQuery()
	{
		...
		$metaQuery
			->select([
				...
				'SUM(IF(' . $db->quoteName('diff') . ' = ' . $db->quote('') . ', 1, 0)) AS ' . $db->quoteName('new'),
				'SUM(IF(' . $db->quoteName('diff') . ' = ' . $db->quote('') . ', 0, 1)) AS ' . $db->quoteName('modified'),
				'SUM(IF(' . $db->quoteName('threat_score') . ' > 0 AND ' . $db->quoteName('acknowledged') . ' = 0, 1, 0)) AS ' . $db->quoteName('suspicious'),
			])
			->from($db->quoteName('#__admintools_scanalerts'))
		...
		$query = $db->getQuery(true)
			...
			->join('left', "($metaQuery) AS " . $db->quoteName('meta'), $db->quoteName('meta.scan_id') . ' = ' . $db->quoteName('s.id'));
		...
		try
		{
			$since = empty($since) ? null : (new Date($since))->toSql();
		}
		catch (\Exception $e)
		{
			$since = null;
		...
		try
		{
			$until = empty($until) ? null : (new Date($until))->toSql();
		}
		catch (\Exception $e)
		{
			$until = null;
