...
namespace Akeeba\Component\AdminTools\Administrator\View\Disallowlists;
...
class HtmlView extends BaseHtmlView
{
	...
	private function addToolbar()
	{
		...
		ToolbarHelper::title(sprintf(Text::_('COM_ADMINTOOLS_TITLE_DISALLOWLISTS')), 'icon-admintools');
		...
		if ($canDelete || $canEditState || $canCreate)
		{
			...
			if ($canCreate)
			{
				$childBar->standardButton('copy', 'COM_ADMINTOOLS_COMMON_LBL_COPY_LABEL')
					->icon('fa fa-copy')
					->task('disallowlists.copy')
					->listCheck(true);
			}

			if ($canDelete)
			{
				$childBar->delete('disallowlists.delete')
